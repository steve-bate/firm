...
async def is_outbox(store: ResourceStore, resource: JSONObject | str) -> bool:
    ...
    return await store.query_one({"outbox": resource_uri}) is not None


async def is_inbox(store: ResourceStore, resource: JSONObject | str) -> bool:
    resource_uri = resource_id(resource)
    return await store.query_one({"inbox": resource_uri}) is not None


async def get_box_owner(store: ResourceStore, resource: JSONObject | str) -> JSONObject | None:
...
class CoreAuthorizationService(AuthorizationService):
    ...
    async def is_get_authorized(
        ...
        if request_actor_uri:
            blocked = await self._is_blocked(tenant, request_actor_uri)
        ...
        if await is_outbox(tenant.public_store, resource):
        ...
        if await is_inbox(tenant.public_store, resource):
            box_owner = await get_box_owner(tenant.public_store, resource)
    ...
    async def _is_blocked(self, tenant: Tenant, request_actor_uri: str) -> AuthorizationDecision:
        # TODO Review blocking
        if blocks := await tenant.private_store.query_one(
            {
            ...
            if blocked_domains := cast(list, blocks.get(FIRM_NS.blockedDomain.value)):
                ...
                if parsed_uri.hostname in blocked_domains:
                    return AuthorizationDecision(False, "inbox post is blocked for domain")
            ...
            if blocked_actors := cast(list, blocks.get(FIRM_NS.blockedActor.value)):
                if request_actor_uri in blocked_actors:
                    return AuthorizationDecision(False, "inbox post is blocked for actor")
        return AuthorizationDecision(True, "Not blocked")

    async def is_post_authorized(
        self, tenant: Tenant, principal: Identity | None, box_type: str, box_uri: str
    ) -> AuthorizationDecision:
        request_actor_uri = principal.uri if principal else None
        ...
        if request_actor_uri:
            if box_type == "inbox":
                ...
                block_decision = await self._is_blocked(tenant, request_actor_uri)
                if not block_decision.authorized:
                    return block_decision

                return AuthorizationDecision(True, "authenticated users can post to inbox")
            ...
            if box_type == "outbox" and principal:
                if principal.actor["outbox"] == box_uri:
                    ...
                    return AuthorizationDecision(False, "only outbox owner can post to it")

        return AuthorizationDecision(False, "authentication required", 401)

    async def is_activity_authorized(
        self, tenant: Tenant, principal: Identity | None, activity: JSONObject
    ) -> AuthorizationDecision:
        ...
        if is_type_any(activity, ["Add", "Remove"]):
            ...
            target = await tenant.public_store.get(resource_id(activity["target"]))
            if (
                principal
                and target
                and (
                ...
                return AuthorizationDecision(True, "Public/owned collection changes allowed")
        elif is_type_any(
            activity,
            ...
            undone_activity = await tenant.public_store.get(undone_activity_uri)
            if undone_activity and is_type_any(undone_activity, ["Follow", "Announce", "Like"]):
            ...
            if self.next_auth:
                ...
                if await self.is_activity_authorized(
                    tenant, principal, {"type": "Create", "object": activity}
                ):
