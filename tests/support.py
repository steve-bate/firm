...
class StubIdentity:
    def __init__(self, data: str | APActor, tenant: Tenant):
...
class StubHttpRequestState(HttpRequestState):
    def __init__(self) -> None:
    ...
    def tenant(self) -> Tenant:
        ...
        if tenant := self._state.get("tenant"):
            return tenant
        else:
            raise ValueError("Tenant not set in request state")
    ...
    def store(self) -> ResourceStore:
        ...
        if store := self._state.get("store"):
            return store
        else:
            raise ValueError("Store not set in request state")

    @property
    def authorizer(self) -> AuthorizationService | None:
        """The authorization service for the tenant."""
        if authorizer := self._state.get("authorizer"):
            return authorizer
        else:
            raise ValueError("Authorizer not set in request state")

    def set(self, key: str, value: Any) -> None:
        """Store arbitrary data on the request state."""
        self._state[key] = value

    def get(self, key: str, default: Any = None) -> Any:
        """Retrieve arbitrary data from the request state."""
...
class StubHttpRequest(HttpRequest):
    def __init__(
        ...
        self._state = StubHttpRequestState()
        if tenant:
            self._state.tenant = tenant
        if auth and auth.tenant:
            self._state.tenant = auth.tenant
