...
from firm.core.interfaces import FIRM_NS, JSONObject, Principal, Tenant
from firm.core.store.memory import MemoryResourceStore
from firm.core.util import AP_PUBLIC_URIS
...
@pytest.fixture
async def principal(tenant: Tenant, tmp_path) -> Principal:
    principal = Principal(
        actor={
            "type": "Person",
    ...
    await tenant.public_store.put(cast(JSONObject, principal.actor))
    return principal
...
async def test_get_not_public_no_authz(
    ...
    authz = CoreAuthorizationService()
    resource: JSONObject = {
        "type": "Note",
        "id": "https://server.test/user/1/note/1",
        "content": "Hello, world!",
    }
    decision = await authz.is_get_authorized(tenant, None, resource)
...
async def test_get_not_public_not_attributed(
    ...
    decision = await authz.is_get_authorized(tenant, principal, resource)
    assert not decision.authorized
...
async def test_post_activity_to_inbox(
    ...
    authz = CoreAuthorizationService()
    decision = await authz.is_post_authorized(tenant, principal, "inbox", inbox_uri)
    assert decision.authorized
...
async def test_post_activity_to_outbox(
    ...
    authz = CoreAuthorizationService()
    decision = await authz.is_post_authorized(tenant, principal, "outbox", outbox_uri)
    assert decision.authorized == authorized
    assert authorized or decision.status_code == status_code


async def test_post_activity_to_outbox_without_auth(tenant: Tenant):
    authz = CoreAuthorizationService()
    decision = await authz.is_post_authorized(tenant, None, "outbox", "https://server.test/box")
    assert not decision.authorized
    assert decision.status_code == HTTPStatus.UNAUTHORIZED.value
...
async def test_activity_add_remove_missing_data(
    ...
    if error == "missing-target":
        activity.pop("target")
    decision = await authz.is_activity_authorized(tenant, principal, activity)
    assert not decision.authorized
...
async def test_activity_add_remove(
    ...
    await tenant.public_store.put(target)
    activity: JSONObject = {
        "type": activity_type,
        "actor": principal.actor["id"],
        "object": "https://server.test/object/1",
...
async def test_instance_level_domain_blocking_post(
    ...
    decision = await authz.is_post_authorized(
        tenant, principal, "inbox", "https://remote.test/inbox"
    )
...
async def test_instance_level_domain_blocking_get(
    ...
    decision = await authz.is_get_authorized(tenant, principal, activity)
